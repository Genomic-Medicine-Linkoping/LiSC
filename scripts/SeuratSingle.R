#!/usr/bin/env Rscript
#Run with R CMD BATCH SeuratSingle.R instead of Rscript to redirect std out to the log file
args = commandArgs(trailingOnly=TRUE)

# Test if there is one argument: if not, return an error
if (length(args) != 10) {
  stop("Other args must be supplied { path/to/filtered_feature_bc_matrix, path/to/results, id, n. min genes per-cell, max genes per-cell, % MT, n. of PCs, cluster resolution (0.4 - 1.2), species (Human|Mouse), tissue (Blood|Brain) }", call.=FALSE)
